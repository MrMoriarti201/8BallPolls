...
public class AS_AccountManagementGUI : MonoBehaviour
{
    ...
    void AccountInfoDownloaded(string message)
    {
		...
		{
			Debug.LogError("Account System: " + message);
			guiMessage = "An error occured while Uploading your info. Try again later!";
		}
		else
		{
			Debug.Log("Account System: " + message + " - Add any custom Logic here!");
    ...
    // Credential Variables - Can Not be initialized here
    // Only in Start / Awake due to player prefs (this
    // is set from the AS_ Setup window

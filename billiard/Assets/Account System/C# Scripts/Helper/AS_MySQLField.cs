...
public static class AS_MySQLFieldHelper
{
	...
	public static bool CheckMySQLFields (this List<AS_MySQLField> fields, ref string errorMessage)
	{
		foreach (AS_MySQLField field in fields)
		{
			...
			if (field.name.ToLower() == "id" || field.name.ToLower() == "isactive" || field.name.ToLower() == "custominfo" || field.name.ToLower() == "username")
				continue;
			
			// Check for ommited fields
			if (field.isRequired && field.stringValue == "")
			{
				errorMessage ="Please input your "+ field.name.UppercaseFirst() + ".";
				return false;
			}
    ...
    private static string FormatAsMySQLString(this AS_MySQLField[] fields)
    {
        ...
        if (fields == null)
        {
            Debug.LogError("AccountSystem: Null argument passed");
    ...
    private static string FormatAsMySQLString(this AS_MySQLField field)
    {
        ...
        if (field.name == "")
        {
            Debug.LogError("AccountSystem: Field doesn't have a name");
        ...
        Debug.Log("AccountSystem: Added field " + field.name + " (of Type: " + field.type.ToString() + ")");

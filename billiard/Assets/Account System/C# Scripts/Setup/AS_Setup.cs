...
public static class AS_Setup {
	...
	{
		if (val == ""){
			//Debug.Log(key + " can not be empty!");
	...
	public static string Setup ( string phpScriptsLocation ){
		...
		Debug.Log("AccountSystem: Setting up PHP scripts..");

		if (phpScriptsLocation == ""){
			Debug.LogError("Php Script Location not set..!");
		...
		if (www.text.ToLower().Contains("error")){
			...
			return "PHP/MySQL Error - Check Debug.Log for more info";
		}
		
		if (www.text.ToLower().Contains("success") ){
			Debug.Log("AccountSystem: PHP Scripts were setup successfully - You can now Initialize the Database!!\n" + www.text);
			return "";
		}
		else{
			Debug.LogWarning("AccountSystem: PHP Scripts could not be set up - Check Message:\n" + www.text);
			return "PHP Scripts could not be set up - Check Debug.Log for more info";
		}		
